...
- **Daily Sales Report Email**: Automated HTML email sent at 3 PM, 5 PM, 8 PM, and 10 PM ET daily to ADMIN, OPERATIONS, EXECUTIVE, and MANAGER users. Contains a table of the day's sales (Rep ID, Rep Name, Account Number, Provider, Service, Gross Commission), total sales count and gross commission amount, and provider mix breakdown by count and percentage. Triggered by background scheduler; also available as manual job type `DAILY_SALES_REPORT`. Always includes `stefanomfischer@gmail.com` as a fixed recipient.
- **Daily Install Report Email**: Automated HTML email sent at 3 PM daily to `stefanomfischer@gmail.com`. Shows all orders from Install Sync runs grouped by status: Installed (Approved), Cancelled, and Pending. Columns: Rep ID, Name, Provider, Service, Customer Name, Account Number, Job Status, Sold Date, Install Date, Install Time, Install Type. Triggered by background scheduler; also available as manual job type `DAILY_INSTALL_REPORT`.
