...
- **Daily Install Report Email**: Automated HTML email sent at 3 PM daily to `ironcrestoperations@ironcrest.ai`. Shows all orders from Install Sync runs grouped by status: Installed (Approved), Cancelled, and Pending. Columns: Rep ID, Name, Provider, Service, Customer Name, Account Number, Job Status, Sold Date, Install Date, Install Time, Install Type. Triggered by background scheduler; also available as manual job type `DAILY_INSTALL_REPORT`.
