...
- **Daily Install Report Email**: Automated HTML email sent at 3 PM daily to `ironcrestoperations@ironcrest.ai`. Shows current month's installs grouped by status: Completed, Cancelled, and Pending. Filters orders by installDate within the current month. Columns: Rep ID, Rep Name, Account #, Invoice #, Customer Name, Provider, Service, Status, Sold Date, Install Date, Order Type (Mobile/Normal). Triggered by background scheduler; also available as manual job type `DAILY_INSTALL_REPORT`.
