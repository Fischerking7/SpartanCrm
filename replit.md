...
- **AI Screenshot-to-Order Capture**: Mobile-first screenshot capture feature on New Order pages (mobile-order-entry and orders dialog). Reps photograph order confirmation screens, Claude vision extracts fields (customer name, address, account number, plan, install date, etc.), and pre-populates the order form. Per-field AI-extracted indicators show confidence levels. Images stored in object storage with role-restricted access (OPERATIONS, ACCOUNTING, ADMIN, EXECUTIVE). Schema columns: `captureMethod` (manual/screenshot_capture enum), `captureImageUrl`, `captureRawJson`. All extraction attempts logged in audit trail. Component: `client/src/components/screenshot-capture.tsx`. Backend: `POST /api/orders/capture`, `GET /api/orders/:id/capture-image`.
...
- **Claude AI (Replit AI Integrations, claude-sonnet-4-6)**: Used for intelligent order matching in the Install Sync feature, and for AI screenshot-to-order field extraction in the order capture module.
