...
- **AI-Powered Order Matching (Install Sync)**: Automates matching installation records against CRM orders using deterministic scoring, with carrier-specific profiles and admin UI. Performance-optimized with pre-indexed order lookups (zip, account, name tokens), cached best-scores to eliminate redundant scoring passes, live progress tracking (`processedRows` column on `install_sync_runs`), and a 30-minute timeout with graceful partial-result persistence.
...
- **Contractor Onboarding System**: OTP-gated portal for 1099 contractors with 7 documents (Background Check, Chargeback Policy, Contractor App, Direct Deposit, Drug Test, NDA, IRS Form W-9), e-signature, PDF generation, admin review, compliance tracking, email notifications to OPS_EMAIL, and audit logging.
