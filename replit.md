...
- **MCP Server (Claude Desktop Connector)**: Standalone MCP server at `server/mcp-server.ts` that exposes read-only CRM data tools for Claude Desktop. Tools include: `get_orders`, `get_order_details`, `search_orders`, `get_reps`, `get_rep_performance`, `get_commissions_summary`, `get_pay_runs`, `get_provider_mix`, `get_chargebacks`, `get_install_sync_runs`, `get_providers`, `get_services`, `get_clients`, `get_dashboard_summary`, and `run_sql_query` (read-only SELECT only). Configuration template at `mcp-config.json`. Requires `DATABASE_URL` pointing to the production database.
...
- **Daily Sales Report Email**: Automated HTML email sent at 3 PM, 5 PM, 8 PM, and 10 PM ET daily to ADMIN, OPERATIONS, EXECUTIVE, and MANAGER users. Contains a table of the day's sales (Rep ID, Rep Name, Account Number, Provider, Service, Gross Commission), total sales count and gross commission amount, and provider mix breakdown by count and percentage. Triggered by background scheduler; also available as manual job type `DAILY_SALES_REPORT`. Always includes `ironcrestoperations@ironcrest.ai` as a fixed recipient.
- **Daily Install Report Email**: Automated HTML email sent at 3 PM daily to `ironcrestoperations@ironcrest.ai`. Shows prior day's installs grouped by status: Completed, Cancelled, and Not Completed. Filters orders by installDate matching yesterday's date. Columns: Rep ID, Name, Provider, Service, Customer Name, Account Number, Job Status, Sold Date, Install Date, Install Time, Install Type. Triggered by background scheduler; also available as manual job type `DAILY_INSTALL_REPORT`.
