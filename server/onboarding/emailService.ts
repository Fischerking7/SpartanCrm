...
export async function sendOnboardingEmails(submissionId: string): Promise<void> {
  try {
    ...
    emailService.queueEmail({
      ...
      text: `${sub.repName} has completed all 6 onboarding documents and is ready for review.\n\nSubmitted: ${new Date(sub.submittedAt).toLocaleString("en-US")}\nIP: ${sub.ipAddress}\n\nPlease review the submission in the admin portal.`,
      html: `<h2>New Contractor Onboarding Submitted</h2><p><strong>${sub.repName}</strong> has completed all 6 onboarding documents and is ready for review.</p><p>Submitted: ${new Date(sub.submittedAt).toLocaleString("en-US")}<br/>IP: ${sub.ipAddress}</p><p>Please review the submission in the admin portal.</p>`,
    });
